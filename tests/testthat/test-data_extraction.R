...
test_that("dataParse handles empty binary data", {
  # Test with empty data
  empty_data <- raw(0)
  result <- dataParse(empty_data)
  expect_equal(length(result), 0)
})

test_that("dataParse validates binary structure", {
  # Test with insufficient data for header
  short_data <- raw(4)  # Less than 8 bytes needed for ChData header
  expect_error(
    dataParse(short_data),
    class = "error"
  )
})

test_that("timeseriesConvert handles different modes", {
  ...
  # Test full mode
  result_full <- timeseriesConvert(mock_parsed, sampling_rate, start_frame, mode = "full")
  expect_equal(nrow(result_full[["1001"]]), 10)
  
  # Test events_only mode
  result_events <- timeseriesConvert(mock_parsed, sampling_rate, start_frame, mode = "events_only")
  expect_true(nrow(result_events[["1001"]]) < 10)  # Should have fewer rows
  expect_true(all(result_events[["1001"]]$voltage != 0))  # No zero values
  
  # Test threshold mode
  result_threshold <- timeseriesConvert(mock_parsed, sampling_rate, start_frame, 
...
test_that("timeseriesConvert calculates time correctly", {
  mock_parsed <- list(
    list(
      channel_id = 2001,
      start_frame = 0,
  ...
  result <- timeseriesConvert(mock_parsed, sampling_rate, start_frame, mode = "full")
...
test_that("timeseriesConvert handles multiple channels", {
  mock_parsed <- list(
    list(
      channel_id = 3001,
      start_frame = 0,
  ...
  result <- timeseriesConvert(mock_parsed, 1000, 0, mode = "full")
...
test_that("timeseriesConvert validates input parameters", {
  # Test invalid mode
  mock_parsed <- list(
  ...
  expect_error(
    timeseriesConvert(mock_parsed, 1000, 0, mode = "invalid_mode"),
    "should be one of"
  )
  
  # Test negative threshold
  result <- timeseriesConvert(mock_parsed, 1000, 0, mode = "threshold", threshold = -10)
